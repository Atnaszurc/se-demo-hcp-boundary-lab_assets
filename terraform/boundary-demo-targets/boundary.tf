...
# Create the HTTP Target in the IT AWS project
resource "boundary_target" "it-http-target" {
  type                     = "tcp"
  name                     = "it-http-target"
...
resource "boundary_alias_target" "it-http-target-alias" {
  ...
  scope_id       = "global"
  value          = "http.boundary.lab"
  destination_id = boundary_target.it-http-target.id
}
